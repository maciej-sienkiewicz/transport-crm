// src/features/drivers/components/DriverActivityHistory/DriverActivityHistory.tsx
...
// src/features/drivers/components/DriverActivityHistory/DriverActivityHistory.tsx (continued)
...
const GoToRouteButton = styled(Button)`
    white-space: nowrap;
`;
...
const categoryOptions = [
    ...
    { value: 'ROUTE', label: driverActivityCategoryLabels.ROUTE },
    { value: 'DRIVER', label: driverActivityCategoryLabels.DRIVER },
    { value: 'VEHICLE', label: driverActivityCategoryLabels.VEHICLE },
    { value: 'CHILD', label: driverActivityCategoryLabels.CHILD },
    { value: 'GUARDIAN', label: driverActivityCategoryLabels.GUARDIAN },
    { value: 'SCHEDULE', label: driverActivityCategoryLabels.SCHEDULE },
];

export const DriverActivityHistory: React.FC<DriverActivityHistoryProps> = ({ driverId }) => {
    const [page, setPage] = useState(0);
    const [category, setCategory] = useState<ActivityCategory | undefined>(undefined);

    const { data, isLoading, error } = useDriverActivities(driverId, {
        category,
        page,
        size: 20,
    });
    ...
    const handleGoToRoute = (routeId: string) => {
    ...
    {category
        ...
        : 'Historia zmian i wydarzeń związanych z tym kierowcą będzie wyświetlana tutaj'}
        </EmptyText>
        </EmptyState>
    ) : (
        <>
            <Timeline>
                {activities.map((activity) => {
                        ...
                        <Badge variant="primary">
                            {driverActivityCategoryLabels[activity.category]}
                            </Badge>
                        {showRouteButton && (
                            <GoToRouteButton
                                variant="secondary"
                            size="sm"
                            onClick={() => handleGoToRoute(activity.metadata!.routeId)}
                        >
                            Przejdź do trasy
                        <ArrowRight size={14} />
                        </GoToRouteButton>
